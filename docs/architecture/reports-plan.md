...
- ~~**No refund / void flow.** `order_status` has `cancelled` and `payment_method` has `refunded`, but there's no built flow, **no cancellation reason, and no who/when/amount audit** for a reversed sale. A genuinely complete audit trail eventually needs void/refund records — flag this as the biggest audit gap.~~
  **RESOLVED** by the Refunds/Void feature (`refunds-plan.md`, Slices 1–5, shipped). `order_refunds` carries the full audit — type, amount, tax portion, CHECK-constrained reason, requested_by/approved_by, timestamp — and every reversal writes a negative `payments` row, so `settledPaymentsWhere()` nets refunds across all reports through one predicate. Reconciliation is proven against seeded refund/void data in `tests/refund_reconciliation_acceptance.mjs`.
- ~~**Cancelled / incomplete orders are invisible.** Every stat filters `status = 'ready'`. For audit *completeness* a report may want to show cancelled orders — but there's no reason captured to explain them.~~
  **RESOLVED** alongside the above: the Transaction Log includes voided orders as flagged rows (net $0), Sales Summary carries a Voids memo, and the Refunds Report gives voids their own audited grain with a captured reason.
